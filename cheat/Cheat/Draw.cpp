...
void TextW(bool cent, unsigned int font, int x, int y, int r, int g, int b, int a, wchar_t *pszString)
{
	...
	g_VGuiSurface->DrawSetTextFont(font);
	g_VGuiSurface->DrawSetTextPos(x, y);
	g_VGuiSurface->DrawPrintText(pszString, (int)wcslen(pszString));
}
...
{
	//g_VGuiSurface->DrawColoredCircle(x, y, radius, r, g, b, a);
	g_VGuiSurface->DrawSetColor(r, g, b, a);
	g_VGuiSurface->DrawOutlinedCircle(x, y, radius, 32);
}
...
void CDraw::DrawTextureByteArray(Vector2D p0, Vector2D p1, Vector2D p2, Vector2D p3, const unsigned char* pRawRGBAData)
{
	...
	g_VGuiSurface->DrawSetTextureRGBA(Texture, buffer/*pRawRGBAData*/, 1, 1);
	...
	verts[2].Init(p2);
	verts[3].Init(p3);
	g_VGuiSurface->DrawTexturedPolygon(4, verts);
}